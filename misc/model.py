...
class AttModel(CaptionModel):
    ...
    def forward(self, img, seq, gt_seq, num, ppls, gt_boxes, mask_boxes, opt, pos_emb=None, spa_adj_matrix=None, sem_adj_matrix=None, eval_opt={}):
        if opt == 'MLE':
            ...
            greedy_result = self._sample(Variable(img.data, volatile=True), Variable(ppls.data, volatile=True), \
                                         Variable(num.data, volatile=True),
                                         {'sample_max': 1, 'beam_size': 1, 'inference_mode': False})
    ...
    def _forward(self, img, seq, ppls, gt_boxes, mask_boxes, num, pos_emd=None, spa_adj_matrix=None, sem_adj_matrix=None):
        ...
        for i in range(self.seq_length):
            ...
            roi_label = utils.bbox_target(ppls, mask_boxes[:, :, :, i + 1], overlaps, seq[:, i + 1],
                                          seq_update[:, i + 1], self.vocab_size)  # roi_label if for the target seq
            # pdb.set_trace()

            roi_labels.append(roi_label.view(seq_batch_size, -1))

            xt = self.embed(it)
            output, det_prob, state = self.core(xt, fc_feats, conv_feats, p_conv_feats, pool_feats, p_pool_feats,
                                                pnt_mask, pnt_mask, state)
    ...
    def _sample(self, img, ppls, num, pos_emb=None, spa_adj_matrix=None, sem_adj_matrix=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            rnn_output, det_prob, state = self.core(xt, fc_feats, conv_feats, p_conv_feats, pool_feats, p_pool_feats,
                                                    att_mask, pnt_mask_list[-1], state)
    ...
    def _sample_beam(self, img, ppls, num, pos_emb, spa_adj_matrix, sem_adj_matrix, opt={}):
        ...
        for k in range(batch_size):
            ...
            if self.cbs:
                self.done_beams[k] = self.constraint_beam_search(state, rnn_output, det_prob, beam_fc_feats,
                                                                 beam_conv_feats, beam_p_conv_feats, \
                                                                 beam_pool_feats, beam_p_pool_feats, beam_ppls,
                                                                 beam_pnt_mask, vis_offset, roi_offset, tag_size, tags,
                                                                 opt)

                ii = 0
                seq[:, k] = self.done_beams[k][tags[-ii - 1]][0][
                    'seq'].cuda()  # the first beam has highest cumulative score
                ...
                self.done_beams[k] = self.beam_search(state, rnn_output, det_prob, beam_fc_feats, beam_conv_feats,
                                                      beam_p_conv_feats, \
                                                      beam_pool_feats, beam_p_pool_feats, beam_ppls, beam_pnt_mask,
                                                      vis_offset, roi_offset, opt)
